...
access_df %>% 
  ...
  select(interview__key,consumed_seafood_qt, all_of(access_vars)) %>% 
  skim()
...
### 1c. Variable importance check
Check variable importance using alternate metrics. 
Note: the model is re-produced here using the 'ranger' rather than the more robust 
final model produced with 'caret' above. 

```{r}
set.seed(1986)
library(ranger)

rf_test1 <- ranger(consumed_seafood_qt ~ ., 
       importance = "impurity",
       mtry = 3,
       num.trees = 5000, 
       data = mod1_df) %>% 
  ...
  as.list() %>% 
  as_tibble() %>% 
  mutate(metric = "impurity")

rf_test2 <- ranger(consumed_seafood_qt ~ ., 
  ...
  as.list() %>% 
  as_tibble() %>% 
  mutate(metric = "impurity_corrected")

rf_test3 <- ranger(consumed_seafood_qt ~ ., 
  ...
  as.list() %>% 
  as_tibble() %>% 
  mutate(metric = "Permutation importance")


rf_test <- bind_rows(rf_test1, rf_test2, rf_test3) %>% 
  pivot_longer(-metric, names_to = "variable", values_to = "score") %>% 
  bind_rows(mod1_rf_imp %>% 
              rename(variable = access_var, score = MeanDecreaseGini) %>% 
              mutate(metric = "Gini importance"))

rf_test %>% 
  filter(metric %in% c("Gini importance","Permutation importance")) %>% 
  ggplot(aes(x = score, y = reorder(variable, score)))+
  geom_bar(stat = "identity")+
  ylab("")+xlab("importance score")+
...
p1 <- mod1_rf_imp %>% 
  left_join(varnames) %>% 
  mutate(varname = str_to_sentence(varname)) %>% 
  ggplot(aes(x = MeanDecreaseGini, y = reorder(varname, MeanDecreaseGini)))+
  geom_bar(stat = "identity")+
  ylab("")+xlab("Gini importance")+
  theme_bw()

p1
```

### 1d. Boxplots of access variables
...
p2 <- access_scaled %>% 
  ...
  mutate(consumed_seafood_qt = recode(consumed_seafood_qt, "high" = "High", "low" = "Low"), 
         consumed_seafood_qt = factor(consumed_seafood_qt, levels = c("Low","High"))) %>%
  left_join(varnames) %>% 
  left_join(mod1_rf_imp) %>% 
  ggplot(aes(y = std_score, x = reorder(varname, MeanDecreaseGini), colour = consumed_seafood_qt))+
...
png(here("MS_access","plots","fig1_mod1_varimp_boxplot.png"), 
...
hsc_mat <- hsc %>% 
  ...
  select(all_of(access_vars)) %>%
  rename_with(.fn = ~gsub("a\\d_","",.)) %>% 
  rename_with(.fn = ~str_to_sentence(.)) %>%  # remove numbers from access var labels
  as.matrix()
...
- Validity of 'majority rule' is contested.
- Relying on commonly used and notably robust metrics
...
hsc_clust %>% 
  ...
  mutate(access_var = factor(access_var, levels = c("technology","labor","knowledge","relationships", 
         ...
         access_var = str_to_sentence(access_var)) %>% 
...
map1 <- vlg_sf %>% 
  ...
  ggplot()+
  geom_raster(data = kir1_bathy_rast, aes(x = x, y = y, fill = depth_m))+
  scale_fill_gradient(name = "Depth (m)")+
  geom_sf(data = kir1_bathy_contour, fill = 'transparent', lwd = .1, color = "grey20")+
  geom_sf(data = kir1, color = "gray30",fill = "gray30", lwd = 2)+  
...
map2 <- vlg_sf %>% 
  ...
  scale_fill_gradient(name = "Depth (m)")+
  geom_sf(data = kir2_bathy_contour, fill = 'transparent', lwd = .1, color = "grey20")+
  geom_sf(data = kir2, color = "gray30",fill = "gray30", lwd = 1)+
  geom_sf(size = 2.2, lwd = 0.05, color = "black", shape = 19, alpha = 1)+
...
png(here("MS_access","plots","Fig3_map.png"), 
